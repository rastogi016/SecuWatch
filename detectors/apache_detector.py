...
def parse_apache_log(log_line):
    ...
    if status == 401:
        ...
        kw in data["path"].lower() for kw in ["select", "union", "drop", "insert"]
    ):
    ...
    is_ip_flagged = (
        isinstance(ip_info, dict) and ip_info.get("abuseConfidenceScore", 0) > 10
    )
    is_url_flagged = isinstance(url_info, dict) and (
        url_info.get("malicious", 0) > 0 or url_info.get("suspicious", 0) > 0
    )
    ...
    if alert_type or is_ip_flagged or is_url_flagged:
        alert_data = {
            ...
            "cti": {"ip_lookup": ip_info, "url_lookup": url_info},
        }
